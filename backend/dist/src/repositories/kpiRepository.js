...
const getEventKpiData = () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    `);
    // 2. Legacy Events
    const legacyRes = yield db_1.default.query(`
        SELECT
    ...
    const populateSlotBD = (rows, source) => {
        for (const r of rows) {
            const key = `${source}-${r.event_id}`;
            if (!slotBreakdownMap[key])
                slotBreakdownMap[key] = {};
            if (!slotBreakdownMap[key][r.slot_date])
                slotBreakdownMap[key][r.slot_date] = {};
