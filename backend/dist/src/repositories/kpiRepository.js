...
let kpiTablePromise = null;
const ensureKpiTable = () => __awaiter(void 0, void 0, void 0, function* () {
    if (kpiTableReady)
        return;
    if (!kpiTablePromise) {
        kpiTablePromise = (() => __awaiter(void 0, void 0, void 0, function* () {
            ...
            try {
                yield db_1.default.query(`CREATE INDEX IF NOT EXISTS idx_kpi_goals_scope ON kpi_goal_settings(scope_type, period_type, period_start)`);
                yield db_1.default.query(`CREATE INDEX IF NOT EXISTS idx_kpi_goals_metric ON kpi_goal_settings(metric_key, period_start)`);
            }
            catch ( /* ok */_b) { /* ok */ }
            kpiTableReady = true;
        }))().finally(() => { kpiTablePromise = null; });
    }
    yield kpiTablePromise;
});
exports.ensureKpiTable = ensureKpiTable;
// ─────────────────────── Dependent Table Init ───────────────────────
let depTablesReady = false;
let depTablesPromise = null;
const ensureDepTables = () => __awaiter(void 0, void 0, void 0, function* () {
    if (depTablesReady)
        return;
    if (!depTablesPromise) {
        depTablesPromise = (() => __awaiter(void 0, void 0, void 0, function* () {
            try {
    ...
    yield depTablesPromise;
});
// ─────────────────────── Helpers ───────────────────────
const buildFunnelFilters = (filters, startIndex = 1) => {
    const conditions = [];
    const params = [];
    let idx = startIndex;
    if (filters.staffId) {
...
const buildDateFilter = (column, filters, params, startIndex) => {
    let idx = startIndex;
    if (filters.month) {
...
// ─────────────────────── Canonical Funnel ───────────────────────
const getCanonicalFunnelCounts = (filters) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (groupByCol) {
        return Object.assign({ applications: 0, reservations: 0, interview_scheduled: 0, interview_completed: 0 }, (filters.groupBy === 'staff' ? { per_staff: result.rows } : { per_source: result.rows }));
    }
...
exports.getCanonicalFunnelCounts = getCanonicalFunnelCounts;
const getDailyApplicationTrend = (filters_1, ...args_1) => __awaiter(void 0, [filters_1, ...args_1], void 0, function* (filters, limit = 62) {
    const { whereClause, params, paramIndex } = buildFunnelFilters(filters);
    let monthCond = '';
    if (filters.month) {
...
exports.getDailyApplicationTrend = getDailyApplicationTrend;
// ─────────────────────── Goals ───────────────────────
const getGoals = (filters) => __awaiter(void 0, void 0, void 0, function* () {
    yield (0, exports.ensureKpiTable)();
    const conditions = [];
    const params = [];
    let idx = 1;
    ...
    const result = yield db_1.default.query(`SELECT * FROM kpi_goal_settings ${where} ORDER BY period_start DESC, metric_key ASC`, params);
    return result.rows;
});
exports.getGoals = getGoals;
const upsertGoals = (goals) => __awaiter(void 0, void 0, void 0, function* () {
    var _a, _b;
    yield (0, exports.ensureKpiTable)();
    for (const g of goals) {
        ...
        `, [g.target_value, g.metric_label || null, g.period_end || null, g.meta ? JSON.stringify(g.meta) : null, g.scope_type || 'global', (_a = g.scope_id) !== null && _a !== void 0 ? _a : null, g.source_company || null, g.period_type || 'monthly', g.period_start, g.metric_key]);
        if (updateResult.rowCount === 0) {
            ...
            `, [g.scope_type || 'global', (_b = g.scope_id) !== null && _b !== void 0 ? _b : null, g.source_company || null, g.period_type || 'monthly', g.period_start, g.period_end || null, g.metric_key, g.metric_label || null, g.target_value, g.meta ? JSON.stringify(g.meta) : '{}']);
        }
    }
});
...
const getEventKpiData = () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    for (const r of statusRes.rows) {
        if (!breakdownMap[r.event_id])
            breakdownMap[r.event_id] = {};
        breakdownMap[r.event_id][r.status] = Number(r.cnt);
    }
    const slotBreakdownMap = {};
    for (const r of slotStatusRes.rows) {
        if (!slotBreakdownMap[r.event_id])
            slotBreakdownMap[r.event_id] = {};
        if (!slotBreakdownMap[r.event_id][r.slot_date])
            slotBreakdownMap[r.event_id][r.slot_date] = {};
        slotBreakdownMap[r.event_id][r.slot_date][r.status] = Number(r.cnt);
    }
    return eventsRes.rows.map((e) => {
...
exports.getEventKpiData = getEventKpiData;
const getSalesActuals = (filters) => __awaiter(void 0, void 0, void 0, function* () {
    yield ensureDepTables();
    const params = [];
    let idx = 1;
    let periodWhere = '';
    if (filters.month) {
        periodWhere = `(TO_CHAR(ps.schedule_date, 'YYYY-MM') = $${idx} OR TO_CHAR(p.entry_deadline, 'YYYY-MM') = $${idx})`;
        params.push(filters.month);
        idx++;
    }
    else if (filters.date) {
        periodWhere = `(TO_CHAR(ps.schedule_date, 'YYYY-MM-DD') = $${idx} OR TO_CHAR(p.entry_deadline, 'YYYY-MM-DD') = $${idx})`;
        params.push(filters.date);
        idx++;
    }
    else if (filters.week) {
        periodWhere = `(TO_CHAR(ps.schedule_date, 'IYYY-\"W\"IW') = $${idx} OR TO_CHAR(p.entry_deadline, 'IYYY-\"W\"IW') = $${idx})`;
        params.push(filters.week);
        idx++;
    }
    const sql = `
