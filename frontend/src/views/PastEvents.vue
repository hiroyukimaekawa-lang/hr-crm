  ...
  <Layout>
    <div class="p-4 md:p-6 lg:p-8 max-w-[1600px] mx-auto">
      ...
      <div class="flex flex-col gap-4 md:flex-row md:items-center justify-between mb-8">
        <div>
          <div class="flex items-center gap-3 mb-1">
            ...
            <h1 class="text-2xl md:text-3xl font-bold text-gray-900 tracking-tight">過去イベント一覧</h1>
          </div>
          <p class="text-sm text-gray-500 mt-1">終了したイベントの実績・参加者を確認できます。</p>
        </div>
      ...
      <div v-if="loading" class="flex flex-col items-center justify-center py-24 gap-4">
        ...
        <p class="text-gray-500 font-medium">イベントデータを読み込み中...</p>
      </div>
      ...
      <div v-else-if="filteredEvents.length === 0" class="bg-white border border-gray-100 rounded-3xl p-12 text-center shadow-sm">
        ...
        <h3 class="text-lg font-bold text-gray-900 mb-1">イベントが見つかりません</h3>
        <p class="text-gray-500 text-sm">検索条件を変更するか、新しいイベントを作成してください。</p>
      </div>
          ...
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="bg-slate-50/80 border-b border-gray-200">
                <th class="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">イベント名</th>
                  ...
                  <td colspan="6" class="px-6 py-0 bg-slate-50/30">
                    <div class="py-6 px-4 border-t border-gray-100">
                      <div class="flex items-center justify-between mb-4 px-2">
                        ...
                        <button
                          @click.stop="router.push(`/events/${event.id}${(event as any).source==='project'?'?source=project':''}`)"
